...
- **Maintenance mode** — one flag in `src/config.ts` (`maintenanceMode`) takes the storefront behind a branded animated page (particles, 6s countdown, Continue Anyway with session bypass) while `/admin` keeps working. `/maintenance` + footer Status link return to it; `/maintenance` redirects home when the flag is off.
