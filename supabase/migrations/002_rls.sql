...
-- Rerun-safe: every policy is dropped first, so this file can run repeatedly.
...
drop policy if exists "public read published drops" on public.drops;
create policy "public read published drops" on public.drops
  for select using (is_published = true or public.is_admin());
drop policy if exists "admin manage drops" on public.drops;
create policy "admin manage drops" on public.drops
  for all using (public.is_admin()) with check (public.is_admin());

drop policy if exists "public read drop products of published drops" on public.drop_products;
...
drop policy if exists "admin manage drop products" on public.drop_products;
...
drop policy if exists "users update own profile (not role)" on public.profiles;
...
drop policy if exists "owner manage addresses" on public.addresses;
create policy "owner manage addresses" on public.addresses
  for all using (user_id = auth.uid() or public.is_admin())
  with check (user_id = auth.uid() or public.is_admin());

drop policy if exists "owner manage cart" on public.cart_items;
create policy "owner manage cart" on public.cart_items
  for all using (user_id = auth.uid()) with check (user_id = auth.uid());

drop policy if exists "owner manage wishlist" on public.wishlists;
...
-- performed via SECURITY DEFINER server functions (003), never by direct update
-- of totals from the client.
drop policy if exists "owner read own orders" on public.orders;
...
drop policy if exists "owner read own order items" on public.order_items;
